...
const model = fakeModel()
  .respond((messages) => {  // [!code highlight]
    const last = messages[messages.length - 1].content as string;  // [!code highlight]
...
Learn how to test your agent with real LLM APIs in [Integration testing](/oss/javascript/langchain/test/integration-testing).
